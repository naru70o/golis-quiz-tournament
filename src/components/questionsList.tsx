    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="collapse-content mt-1">
          <div className="flex gap-2 items-center justify-end mb-2">
            <div className="Lucide-icon" onClick={()=>showModal("question_updateForm_model")}>
              <Pencil className=""/>
              <UpdateQuestionForm question={question}/>
            </div>
            <div className="Lucide-icon" onClick={()=>showModal("delete_model")}>
              <Trash/>
              <DeleteModal message="Question" deleteHandler={()=>deleteQuestion(_id)}/>
            </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-y-3 gap-x-3 grid-cols-1">
          ...
          {options.map((option: Option,index:number)=>(
            <div className="px-4 py-2 bg-red-400 rounded-xl relative overflow-clip w-full" key={option.text}>
            <div className="absolute rounded-xl bg-red-600 px-4 py-2 left-0 top-0 h-full text-3xl font-bold">{optionIdentifier(index)}</div>  
